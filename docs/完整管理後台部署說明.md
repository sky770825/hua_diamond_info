...
6. 在 **Cloudflare Pages** 專案設定環境變數：
   - `VITE_API_URL` = 後端網址（不含結尾 `/`）
   - `VITE_ADMIN_URL` = 後端網址
7. 重新部署前端（push 或手動觸發）

**注意**：後端目前使用本機檔案（`data/members.json`、`uploads/`）。Zeabur 容器重啟後資料可能遺失。若要持久化，需改接 PostgreSQL 或雲端儲存。

---

## 四、目前後端狀態

- **資料儲存**：`backend/data/members.json`（本機檔案）
- **上傳儲存**：`backend/uploads/`（本機目錄）
- **Zeabur 部署**：需改為雲端儲存（例如 R2、S3）或資料庫，否則重啟後資料會消失
